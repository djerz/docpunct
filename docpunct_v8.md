# docpunct — Specification v8

This version incorporates implementation decisions made during the initial
Codex implementation sessions through the session that added repeatable
ShellCheck and container tests, latest-release Double Commander support,
desktop application grouping, fd-find compatibility handling, and Docker group
membership verification. It also records the session that fixed Cargo
environment handling for same-run Rust/Neovide installs and added guarded
Cargo/nvm shell initialization to the imported `.bashrc`, plus the session that
fixed Neovide installation coverage and dependency traversal after a real
Neovide install failure.

Version 8 records the sessions that:
