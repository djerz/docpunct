# docpunct — Specification v5

This version incorporates implementation decisions made during the initial
Codex implementation session and the follow-up session that added dependency
cycle detection, updated Debian GUI package handling, and introduced separate
third-party APT repository features.

Version 5 records the session that:
...
- prepared the next testing step: running ShellCheck through a Docker image and
  adding a `just shellcheck` target
